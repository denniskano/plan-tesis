...
el \textbf{\ServiceDomain{} BIAN} (\Behavior{}, \BusinessObject{}, relaciones de control). El
diagnóstico es de \textbf{correspondencia}: mismos conceptos con nombres distintos, operaciones HTTP
que no reflejan \textit{behaviors}, esquemas que no cubren \textit{business objects}, versiones sin
trazabilidad (§1.2; Tabla~\ref{tab:tipologias}).

En este nivel se fija el \textbf{objeto de estudio} (contrato OpenAPI bancario completo) y la
\textbf{unidad de análisis} (pares endpoint/\allowbreak schema $\leftrightarrow$
\Behavior{}/\BusinessObject{}; §1.4). BIAN deja de ser «el banco» y funciona como \textbf{modelo de
referencia} documental frente a la capa REST.
...
abstracta. Es la \textbf{inexistencia de un método reproducible} que, dado un contrato OpenAPI
bancario y un \ServiceDomain{} BIAN emparejado, calcule de forma sistemática en qué medida se
alinean y bajo qué tipologías se desalinea. Hoy esa evaluación es manual, parcial o simplemente no
ocurre antes del despliegue.

Este plan (Guía~1) responde en el nivel de la \textbf{necesidad del artefacto}: diseño del
...
explica por qué la literatura aún no lo resuelve ---aunque no debe confundirse con la fila de
problema de Anexo~C, como insistió el docente.

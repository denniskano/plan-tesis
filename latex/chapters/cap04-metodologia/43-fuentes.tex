...
``señales'' documentales (PI-11). Cada instancia debe identificarse de forma unívoca (p. ej.
\texttt{\{fuente\}\_\{service-domain\}\_\{version\}}).

\input{tables/tab04-adquisicion-datos}

\subsection{Criterios de inclusión (muestra empírica, Tesis~2)}

Para la validación futura se considerarán contratos OpenAPI~3.x del sector bancario emparejados
con un \ServiceDomain{} BIAN declarado en la documentación del caso, con:
...
OpenAPI $\leftrightarrow$ BIAN en ese universo; la \textbf{muestra} será el subconjunto de
contratos/pares seleccionados en Tesis~2 según representatividad de dominios (pagos, cuentas,
préstamos, etc.), no un piloto único obligatorio.
